...
const UserSubscription = () => {
  ...
  useEffect(() => {
    const fetchPlans = async () => {
      try {
        // جلب الخطط العادية
        const response = await axios.get("http://localhost:4000/api/subscriptions/user/user-subscription-plans");
        setPlans(response.data);

        // جلب الخطط المحذوفة
        const deletedPlansResponse = await axios.get("http://localhost:4000/api/subscriptions/user/user-subscription-plans/deleted/all");
        setDeletedPlans(deletedPlansResponse.data);
  ...
  const handleAddPlan = async (e) => {
    ...
    try {
      if (editingPlan) {
        // Update existing plan
        const response = await axios.put(`http://localhost:4000/api/subscriptions/user/user-subscription-plans/${editingPlan.id}`, newPlan);
        const updatedPlan = response.data;
        setPlans(plans.map((plan) => (plan.id === updatedPlan.id ? updatedPlan : plan)));
      } else {
        // Add new plan
        const response = await axios.post("http://localhost:4000/api/subscriptions/user/user-subscription-plans", newPlan);
        const addedPlan = response.data;
        setPlans([...plans, addedPlan]);
      }
     setNewPlan({
  ...
  const handleDeletePlan = async (planId) => {
    ...
    try {
      await axios.delete(`http://localhost:4000/api/subscriptions/user/user-subscription-plans/${planId}`);
      setPlans(plans.filter((plan) => plan.id !== planId));
    } catch (error) {
      console.error("Error during delete:", error);
      setError("Failed to delete plan.");
  ...
  const handleRestorePlan = async (planId) => {
    ...
    try {
      const response = await axios.put(`http://localhost:4000/api/subscriptions/user/user-subscription-plans/restore/${planId}`);
      ...
      >
        ...
        {formVisible && (
          ...
          >
            ...
            {/* Discount Count */}
            <div>
              <label className="block text-gray-700 mb-2">Discount Bookings Count</label>
              <input
                type="number"
                value={newPlan.discount_bookings_count}
                onChange={e => setNewPlan({...newPlan, discount_bookings_count: e.target.value})}
                className="w-full p-3 border rounded-lg"
                required
              />
            </div>
            ...
            <div className="flex items-center">
              <input
                type="checkbox"
                checked={newPlan.ai_enabled}
                onChange={e => setNewPlan({...newPlan, ai_enabled: e.target.checked})}
                id="ai_enabled"
                className="mr-2"
              />
              <label htmlFor="ai_enabled" className="text-gray-700">Enable AI Access</label>
            </div>
            {/* AI Duration */}
            {newPlan.ai_enabled && (
              <div>
                <label className="block text-gray-700 mb-2">AI Duration (days)</label>
                <input
                  type="number"
                  value={newPlan.ai_duration_days}
                  onChange={e => setNewPlan({...newPlan, ai_duration_days: e.target.value})}
                  className="w-full p-3 border rounded-lg"
                  required
                />
              </div>  )}
                    ...
                    {deletedPlans.map((plan) => (
                      <tr key={plan.id} className="hover:bg-gray-50">
                        ...
                        <td className="px-6 py-4 whitespace-nowrap">
                         ...
                         <td className="px-6 py-4 whitespace-nowrap">
                <div className="text-gray-500">{plan.discount_percentage}%</div>
              </td>
              <td className="px-6 py-4 whitespace-nowrap">
                <div className="text-gray-500">{plan.discount_bookings_count}</div>
              </td>
              <td className="px-6 py-4 whitespace-nowrap">
                <div className="text-gray-500">
                  {plan.ai_enabled ? plan.ai_duration_days : '-'}
                </div>
              </td>
                        <td className="px-6 py-4">
