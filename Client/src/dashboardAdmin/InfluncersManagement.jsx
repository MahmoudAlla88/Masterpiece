...
const InfluencersManagement = () => {
  ...
  const filteredInfluencers = influencers.filter(influencer => {
    ...
    if (statusFilter !== 'all' && i.user.adminApproved !== statusFilter) return false;
    if (!searchQuery) return true;
    const q = searchQuery.toLowerCase();
    return (
      i.user.name?.toLowerCase().includes(q) ||
      i.user.email?.toLowerCase().includes(q) ||
      i.influencerRegistration.contentCategories?.some(cat => cat.toLowerCase().includes(q))
    );
  });
const totalPages = Math.ceil(filtered.length / itemsPerPage);
  const start = (currentPage - 1) * itemsPerPage;
  const currentInfluencers = filtered.slice(start, start + itemsPerPage);
